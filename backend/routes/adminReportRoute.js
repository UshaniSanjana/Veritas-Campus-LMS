...
router.get('/logins', async (req, res) => {
  try {
    const [enrollments, student, instructor, passwordChanges, courses] = await Promise.all([
      Enrollment.aggregate([
        { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$createdAt" } }, enrollments: { $sum: 1 } } }
      ]),
      Student.aggregate([
        { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$createdAt" } }, student: { $sum: 1 } } }
      ]),
      Instructor.aggregate([
        { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$createdAt" } }, instructor: { $sum: 1 } } }
      ]),
      PasswordChange.aggregate([
        { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$createdAt" } }, passwordChanges: { $sum: 1 } } }
      ]),
      Course.aggregate([
        { $group: { _id: { $dateToString: { format: "%Y-%m-%d", date: "$createdAt" } }, courses: { $sum: 1 } } }
      ]),
    ]);
    ...
    [...enrollments, ...student, ...instructor, ...passwordChanges, ...courses].forEach(item => {
      const date = item._id;
      if (!map[date]) {
        map[date] = { date, enrollments: 0, student: 0, instructor: 0, passwordChanges: 0, courses: 0 };
      }
      if (item.enrollments !== undefined) map[date].enrollments = item.enrollments;
      if (item.student !== undefined) map[date].student = item.student;
      if (item.instructor !== undefined) map[date].instructor = item.instructor;
      if (item.passwordChanges !== undefined) map[date].passwordChanges = item.passwordChanges;
