...
router.get('/progress', async (req, res) => {
  try {
    // Total counts
    const [totalAssignments, totalQuizzes, totalLectures, totalMaterials, totalExams] = await Promise.all([
    ...
    const [completedLectures, completedMaterials, completedExams] = await Promise.all([
    ...
    const totalItems = totalAssignments + totalQuizzes + totalLectures + totalMaterials + totalExams;
    // Completed items
    const completedItems = completedAssignments + completedQuizzes + completedLectures + completedMaterials + completedExams;
