...
router.put('/updateStatus/:id', async (req, res) => {
  ...
  try {
    let updatedRequest = await StudentSupport.findByIdAndUpdate(id, { status }, { new: true });

    if (!updatedRequest) {
      updatedRequest = await InstructorSupport.findByIdAndUpdate(id, { status }, { new: true });
    }
