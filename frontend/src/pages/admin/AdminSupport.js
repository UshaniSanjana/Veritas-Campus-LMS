          ...
          {requests.length === 0 ? (
            ...
            requests.map((req) => (
              <tr key={req._id}>
                ...
                <td>
                  <span className={`status-label ${req.status ? req.status.toLowerCase().replace(/\s/g, '-') : 'pending'}`}>
                    {req.status || 'Pending'}
                  </span>
                </td>
                <td>
                ...
                <td>
                  <button
                    className="v-btn"
                    onClick={() => navigate(`/admin/singleViewSupport/${req._id}`)}
                  >
                    View
                  </button>
                  <select
...
const AdminSupport = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5000/api/adminsupport/students')
      .then(res => setStudentRequests(res.data))
      .catch(err => console.error(err));

    axios.get('http://localhost:5000/api/adminsupport/instructors')
      .then(res => setInstructorRequests(res.data))
      .catch(err => console.error(err));
  }, []);

  const handleStatusChange = (id, newStatus) => {
    axios.put(`http://localhost:5000/api/adminsupport/updateStatus/${id}`, { status: newStatus })
      .then(res => {
  ...
  const filteredStudentRequests = studentRequests.filter(req =>
    (req.studentName || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
    (req.status || '').toLowerCase().includes(searchQuery.toLowerCase())
  );

  const filteredInstructorRequests = instructorRequests.filter(req =>
    (req.lectureName || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
    (req.status || '').toLowerCase().includes(searchQuery.toLowerCase())
  );
      ...
      <div className='body'>
        ...
        <div className='header'>
          ...
          <button className="abtn" onClick={downloadPageAsPDF}>
            <MdDownload size={17} style={{ verticalAlign: "middle" }} /> Export as PDF
          </button>
        </div>
