...
const SupportList = () => {
  ...
  const fetchSupportsRequests = async () => {
    try {
      ...
      const res = await axios.get(
        "https://veritas-campus-lms-production.up.railway.app/api/instructor/support"
      );
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this request?")) {
      try {
        await axios.delete(
          `https://veritas-campus-lms-production.up.railway.app/api/instructor/support/${id}`
        );
      ...
      <Container fluid className="py-4">
                ...
                <Card className="h-100 shadow-sm">
                  ...
                  {request.photo && (
                    <div className="position-relative">
                      <Card.Img
                        variant="top"
                        src={`https://veritas-campus-lms-production.up.railway.app/uploads/instructorsupport${request.photo}`}
                        alt="Issue"
                        className="img-fluid"
                        onClick={() => handleView(request)}
                      />
                      <div
                        className="position-absolute top-0 end-0 p-2 text-white bg-dark bg-opacity-50"
                    ...
                    {request.status === "replied" && (
                      <div className="mt-3 border-top pt-2">
                        <p className="fw-bold mb-1">
                          <i className="bi bi-reply-fill me-2"></i>Admin
                          Response
                        </p>
        ...
        >
          <Modal.Header closeButton>
            <Modal.Title>
              <i className="bi bi-ticket-detailed-fill me-2"></i>Support Request
              Details
              {selectedRequest && (
                <span className="ms-2">
            ...
            {selectedRequest && (
              <>
                <div className="mb-3">
                  ...
                  <p className="text-muted">
                    Lecture ID: {selectedRequest.lectureID}
                  </p>
                ...
                <div className="mb-3">
                  <h6>
                    <i className="bi bi-chat-text-fill me-2"></i>Issue
                    Description
                  </h6>
                ...
                {selectedRequest.photo && (
                  <div className="mb-3">
                    ...
                    <img
                      src={`https://veritas-campus-lms-production.up.railway.app/uploads/instructorsupport${selectedRequest.photo}`}
                      alt="Evidence"
                      className="img-fluid rounded"
                    />
                    <a
                      href={`https://veritas-campus-lms-production.up.railway.app/uploads/instructorsupport${selectedRequest.photo}`}
                      target="_blank"
                ...
                {selectedRequest.status === "replied" && (
                  <div className="mt-4 border-top pt-3">
                    <h6>
                      <i className="bi bi-reply-fill me-2"></i>Admin Response
                    </h6>
                    <p>{selectedRequest.adminReply.message}</p>
                    <small className="text-muted">
                      By: {selectedRequest.adminReply.adminName} at{" "}
