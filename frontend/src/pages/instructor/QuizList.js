...
const QuizList = () => {
    ...
    const fetchQuizzes = async () => {
      try {
        const response = await axios.get(
          "https://veritas-campus-lms-production.up.railway.app/api/instructor/quiz"
        );
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(
        `https://veritas-campus-lms-production.up.railway.app/api/instructor/quiz/${deleteTarget.id}`
      );
        ...
        <div className="d-flex align-items-center">
          ...
          <Link to="/instructor/addquiz" className="ms-auto">
            <button
              ...
              style={{ backgroundColor: "#7AC144", border: "none" }}
            >
              Add Quiz
            </button>
          </Link>
