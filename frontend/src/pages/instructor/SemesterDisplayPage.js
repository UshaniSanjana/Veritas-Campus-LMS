...
const SemesterDisplayPage = () => {
  ...
  useEffect(() => {
    const fetchSemesters = async () => {
      ...
      try {
        ...
        // const response = await fetch(`https://veritas-campus-lms-production.up.railway.app/api/instructor/semesters?year=${year}`);
  ...
  if (loading)
    return <div className="container mt-4">Loading semesters...</div>;
  if (error) {
    return (
      <div className="container mt-4">
        <div className="alert alert-danger">Error: {error}</div>
        <Link to="/instructor/years" className="btn btn-primary">
          Back to Years
        </Link>
      </div>
    );
      ...
      <div className="semester-list-container">
        <div className="semester-item mb-3">
          <Link
            ...
            onClick={() =>
              console.log(
                `Navigating to modules for Year ${year}, All Semesters`
              )
            }
          >
        ...
        {semesters.map((semester) => (
          <div key={semester} className="semester-item mb-3">
            <Link
              ...
              onClick={() =>
                console.log(
                  `Navigating to modules for Year ${year}, Semester ${semester}`
                )
              }
            >
              <i className="fas fa-chevron-right me-2"></i>
