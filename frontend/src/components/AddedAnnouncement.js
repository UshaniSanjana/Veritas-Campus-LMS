...
const AddedAnnouncement = () => {
  ...
  useEffect(() => {
    fetch(
      "https://veritas-campus-lms-production.up.railway.app/addedannouncement"
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDownload = (id, hasFile) => {
    if (hasFile) {
      window.open(
        `https://veritas-campus-lms-production.up.railway.app/download/${id}`,
        "_blank"
      );
    }
  };
  ...
  const handleDeleteConfirmed = async (id) => {
    await fetch(
      `https://veritas-campus-lms-production.up.railway.app/announcement/${deleteId}`,
      { method: "DELETE" }
    );
  ...
  const filtered = Array.isArray(announcements)
    ? announcements.filter(
        (a) => a.title && a.title.toLowerCase().includes(filter.toLowerCase())
      )
    : [];
                  ...
                  <div className="description-container">
                    <div
                      ...
                      className={`description-text ${
                        expandedDescriptions[a._id] ? "expanded" : ""
                      }`}
                    >
                      {a.message}
                    </div>
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <p style={{ fontSize: "1.3rem", marginBottom: 32 }}>
              Are you want to Delete Announcement{" "}
              {announcements.find((a) => a._id === deleteId)?.originalIndex}?
            </p>
